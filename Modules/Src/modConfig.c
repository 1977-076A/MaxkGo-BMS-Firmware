...
void modconfigHardwareLimitsApply(modConfigGeneralConfigStructTypedef *configLocation)
{
	...
	if ((configLocation->cellMonitorICCount % configLocation->noOfParallelModules) == 0)
	{ // Check if feasable configuration
...
void modConfigLoadDefaultConfig(modConfigGeneralConfigStructTypedef *configLocation)
{
	...
	configLocation->batteryCapacity = 5.20f;									// XXAh battery
	...
	configLocation->chargerEnabledThreshold = 5.5f;								// If charge current > X.XA stay in charging mode and dont power off.
